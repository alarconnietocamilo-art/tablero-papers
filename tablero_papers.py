...
if submit_btn:
    # Determinar el nombre final del paper
    ...
    if not paper_final or paper_final.strip() == "":
        ...
        # Formatear fechas estrictamente como texto para cuidar la compatibilidad en Excel
        ...
        st.success("✅ Avance sincronizado correctamente.")
        st.rerun()
4. Despliegue de Indicadores con Sistema de Pestañas (Tabs)
...
with tab_individual:
    ...
    # Lógica de progreso: Buscar el estado más reciente de cada actividad registrada
    ...
    with col_lista:
        ...
        # Mostrar la estructura completa de las 9 actividades para ver qué falta
        for act in ACTIVIDADES:
            if act in actividades_completadas:
                st.markdown(f"✅ ~~{act}~~")
            elif act in df_latest['Actividad'].tolist():
                estado_actual = df_latest[df_latest['Actividad'] == act]['Estado'].values[0]
